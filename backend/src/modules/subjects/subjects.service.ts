...
export class SubjectsService {
    ...
    async create(createSubjectDto: CreateSubjectDto, userRole: string, referenceId: string) {
        ...
        if (!teacher.assignedClassId) {
            throw new ForbiddenException('You must have an assigned class to create subjects');
        }
    ...
    async findOne(id: string, userRole: string, referenceId: string) {
        ...
        if (userRole === UserRole.TEACHER && subject.teacherId.toString() !== referenceId) {
    ...
    async update(id: string, updateSubjectDto: UpdateSubjectDto, userRole: string, referenceId: string) {
        const subject = await this.subjectModel.findById(id);
        if (!subject) {
            throw new NotFoundException('Subject not found');
        }
