...
function StudentDetailPage() {
    ...
    const presentDays = attendance.filter((a: any) => a.isPresent || a.status === 'present').length;
    const attendancePct = totalDays > 0 ? Math.round((presentDays / totalDays) * 100) : 0;

    // Compute marks overall percentage
    let totalObtained = 0;
    let totalMax = 0;
    marks.forEach((m: any) => {
    ...
    const overallMarksPct = totalMax > 0 ? Math.round((totalObtained / totalMax) * 100) : 0;
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-page)', paddingTop: '52px' }}>
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto', padding: '24px 16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {/* Attendance Simple Widget */}
                    <div style={{ background: 'var(--bg-card)', borderRadius: '14px', padding: '20px', boxShadow: 'var(--shadow-sm)' }}>
                        <h3 style={{ margin: '0 0 14px', fontSize: '16px', color: 'var(--text-main)' }}>📅 Daily Attendance Record</h3>
                        <StudentAttendanceStats studentId={id || ''} />
                    </div>
